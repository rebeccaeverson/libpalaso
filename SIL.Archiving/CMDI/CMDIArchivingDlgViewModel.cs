...
{
	/// <summary>Implements archiving for CMDI repositories</summary>
	public class CMDIArchivingDlgViewModel : ArchivingDlgViewModel, ISupportMetadataOnly
	{
		private readonly CMDIPackage _cmdiData;
		...
		private readonly string _configFileName = Path.Combine(ArchivingFileSystem.SilCommonArchivingDataFolder, "CMDIProgram.config");
		...
		internal override string ArchiveType
		{
			get
			{
				return LocalizationManager.GetString("DialogBoxes.ArchivingDlg.CMDIArchiveType", "CMDI",
					"This is the abbreviation for Isle Metadata Initiative (http://www.mpi.nl/cmdi/). " +
		...
		public override string InformativeText
		{
			get
			{
				string programInfo = string.IsNullOrEmpty(NameOfProgramToLaunch) ?
					string.Format(LocalizationManager.GetString("DialogBoxes.ArchivingDlg.NoCMDIProgramInfoText",
					"The {0} package will be created in {1}.",
					"Parameter 0 is 'CMDI'; " +
					"Parameter 1 is the path where the package is created."),
					ArchiveType, PackagePath)
					:
					string.Format(LocalizationManager.GetString("DialogBoxes.ArchivingDlg.CMDIProgramInfoText",
					...
					"Parameter 0 is the name of the program that will be launched to further prepare the CMDI data for submission; " +
					"Parameter 1 is the name of the calling (host) program (SayMore, FLEx, etc.)"), NameOfProgramToLaunch, AppName);
				return string.Format(LocalizationManager.GetString("DialogBoxes.ArchivingDlg.CMDIOverviewText",
		...
		{
			get { return Properties.Settings.Default.CMDIWebSite; }
		}
		...
		/// <param name="outputFolder">Base folder where CMDI file structure is to be created</param>
		/// ------------------------------------------------------------------------------------
		public CMDIArchivingDlgViewModel(string appName, string title, string id,
			...
			: base(appName, title, id, appSpecificArchivalProcessInfo, setFilesToArchive)
		{
			...
			_cmdiData = new CMDIPackage(corpus, PackagePath)
			{
				Title = _titles[_id],
				Name = _id
			};
		...
		protected override string FileGroupDisplayMessage(string groupKey)
		{
			if (groupKey == string.Empty)
				return LocalizationManager.GetString("DialogBoxes.ArchivingDlg.CMDIActorsGroup", "Actors",
					"This is the heading displayed in the Archive Using CMDI dialog box for the files for the actors/participants");
		...
		public void SetSessionDescription(string sessionId, string description, string iso3LanguageCode)
		{
			...
			_cmdiData.AddDescription(sessionId, new LanguageString { Value = description, Iso3LanguageId = iso3LanguageCode });
		}
		...
		protected override void SetAbstract_Impl(IDictionary<string, string> descriptions)
		{
			foreach (var desc in descriptions)
				_cmdiData.AddDescription(new LanguageString(desc.Value, desc.Key));
		}
		...
		{
			return _cmdiData.BaseCmdiFile.ToString();
		}
		...
		internal override void LaunchArchivingProgram()
		{
			...
			{
				ReportError(e, string.Format(LocalizationManager.GetString("DialogBoxes.ArchivingDlg.StartingCMDIErrorMsg",
					"There was an error attempting to open the archive package in {0}."), PathToProgramToLaunch));
			}
		}

#region Create CMDI package in worker thread
		...
		public override bool CreatePackage()
		{
			...
			var success = _cmdiData.SetMissingInformation();

			// write the xml files
			if (success)
				success = _cmdiData.CreateCMDIPackage();

			// copy the content files
			if (success && !MetadataOnly)
				success = CreateCMDIPackage();

			CleanUp();

			if (success)
			{
				// copy the path to the cmdi file to the clipboard

				// SP-818: Crash in CMDI export when dialog tries to put string on clipboard
				...
				//Clipboard.SetData(DataFormats.Text, _cmdiData.MainExportFile);
				Clipboard.SetDataObject(_cmdiData.MainExportFile, true, 3, 500);

				var successMsg = LocalizationManager.GetString("DialogBoxes.ArchivingDlg.ReadyToCallCMDIMsg",
					"Exported to {0}. This path is now on your clipboard. If you are using Arbil, go to File, Import, then paste this path in.");
				DisplayMessage(string.Format(successMsg, _cmdiData.MainExportFile), MessageType.Success);
			}
		...
		public bool CreateCMDIPackage()
		{
			try
			{
				using (_worker = new BackgroundWorker())
				{
					...
					_worker.DoWork += CreateCMDIPackageInWorkerThread;
			...
			catch (Exception e)
			{
				ReportError(e, LocalizationManager.GetString(
					"DialogBoxes.ArchivingDlg.CreatingCMDIPackageErrorMsg",
					"There was a problem starting process to create CMDI package."));
		...
		private void CreateCMDIPackageInWorkerThread(object sender, DoWorkEventArgs e)

		{
			try
			{
				var outputDirectory = Path.Combine(_cmdiData.PackagePath, NormalizeDirectoryName(_cmdiData.Name));

				if (Thread.CurrentThread.Name == null)
					Thread.CurrentThread.Name = "CreateCMDIPackageInWorkerThread";
				...
				foreach (var sess in _cmdiData.Sessions)
				{
					...
					if (FileCopyOverride != null)
					{
						...
						catch (Exception error)
						{
							var msg = string.Format(LocalizationManager.GetString("DialogBoxes.ArchivingDlg.FileExcludedFromPackage",
								"File excluded from {0} package: ", "Parameter is the type of archive (e.g., RAMP/CMDI)"), ArchiveType) +
								fileToCopy.Value;
				...
				_worker.ReportProgress(0, string.Format(LocalizationManager.GetString("DialogBoxes.ArchivingDlg.SavingFilesInPackageMsg",
					"Saving files in {0} package", "Parameter is the type of archive (e.g., RAMP/CMDI)"), ArchiveType));
			}
			catch (Exception exception)
			{
				_worker.ReportProgress(0, new KeyValuePair<Exception, string>(exception,
					string.Format(LocalizationManager.GetString("DialogBoxes.ArchivingDlg.CreatingArchiveErrorMsg",
						"There was an error attempting to create the {0} package.", "Parameter is the type of archive (e.g., CMDI)"), ArchiveType)));
		...
		public override IArchivingSession AddSession(string sessionId)
		{
			...
			foreach (var sess in _cmdiData.Sessions.Where(sess => sess.Name == sessionId))
				return sess;

			// if not found, add a new session
			Session session = new Session {Name = sessionId};

			_cmdiData.Sessions.Add(session);

			return session;
		}

		public override IArchivingPackage ArchivingPackage { get { return _cmdiData; } }
