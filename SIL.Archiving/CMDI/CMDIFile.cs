...
namespace SIL.Archiving.CMDI
{
	...
	public class CMDIFile : ArchivingFile
	{
		...
		public CMDIFile(string fullFileNameAndPath) : base(fullFileNameAndPath)
		{
		}

		/// <summary>Constructor</summary>
		public CMDIFile(ArchivingFile file) : base(file)
		{
		}
		...
		public string NormalizedName
		{
			get
			{
				...
				{
					_normalizedName = CMDIArchivingDlgViewModel.NormalizeFileName(FileName);
				}
		...
		/// <summary>Convert to MediaFile_Type for CMDI meta data file</summary>
		...
		/// <summary>Convert to WrittenResource_Type for CMDI meta data file</summary>
		public WrittenResourceType ToWrittenResourceType(string sessionDirectoryName)
		{
			...
			if (!string.IsNullOrEmpty(DescribesAnotherFile))
				written.MediaResourceLink = new ResourceLinkType {
					...
					CMDIArchivingDlgViewModel.NormalizeFileName((new FileInfo(DescribesAnotherFile)).Name ))
				};
			else
		...
		/// <param name="sessionDirectoryName"></param>
		private void SetResourceProperties(ICMDISessionFile resource, string sessionDirectoryName)
		{
			...
			foreach (var description in Descriptions)
				resource.Description.Add(description.ToCMDIDescriptionType());
