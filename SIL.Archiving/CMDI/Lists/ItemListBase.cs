...
namespace SIL.Archiving.CMDI.Lists
{
	/// <summary>Generic class to handle items in the CMDI lists</summary>
	public class CMDIListItem : IComparable
	{
		...
		public CMDIListItem(string value, string definition)
		{
			Text = value;
			Value = value;
		...
		/// <summary>Used for sorting. Compare 2 CMDIListItem objects. They are identical if the Text properties are the same</summary>
		public int CompareTo(object obj)
		{
			if (obj == null)
				return 1;

			CMDIListItem other = obj as CMDIListItem;
		...
		/// <summary>Used for sorting. Compare 2 CMDIListItem objects. They are identical if the Text properties are the same</summary>
		public static int Compare(CMDIListItem itemA, CMDIListItem itemB)
		{
			return itemA.CompareTo(itemB);
		}

		/// <summary>This allows forcing specific entries to the top of the sort order</summary>
		private static string CheckSpecialCase(CMDIListItem inputValue)
		{
	...
	/// <summary>
	/// A list of CMDI list items, constructed using the cmdi:Entry Nodes from the XML file.
	/// <example>
	/// Example 1:
	/// ----------
	/// var lc = CMDI_Schema.ListConstructor.GetList(ListType.MPILanguages);
	...
	/// </example>
	/// </summary>
	public class CMDIItemList : List<CMDIListItem>
	{
		...
		/// ---------------------------------------------------------------------------------------
		/// <summary>Constructs a list of CMDIListItems that can be used as the data source of a
		/// combo or list box</summary>
		/// <param name="listName">Name of the XML file that contains the desired list. It is suggested to
		///     use values from CMDI_Schema.ListTypes class. If not found on the local system, we will attempt
		///     to download from http://www.mpi.nl/CMDI/Schema.
		/// </param>
		/// <param name="uppercaseFirstCharacter">Make first character of each item uppercase</param>
		internal CMDIItemList(string listName, bool uppercaseFirstCharacter) 
		...
		internal CMDIItemList(string listName, bool uppercaseFirstCharacter, ListConstructor.RemoveUnknown removeUnknown)
		{
		...
		{
			// Add blank option
			Insert(0, new CMDIListItem(string.Empty, string.Empty));
		}
		...
		private XmlNodeList GetNodeList(string listName)
		{
			...
			nsmgr.AddNamespace("cmdi", "http://www.mpi.nl/CMDI/Schema/CMDI");
			...
			var nodes = doc.DocumentElement.SelectNodes("//cmdi:VocabularyDef/cmdi:Entry", nsmgr);
		...
		private static string ListPath
		{
			get
			{
				...
				var thisPath = ArchivingFileSystem.SilCommonCMDIDataFolder;

				// check if path exists
				if (!Directory.Exists(thisPath))
					throw new DirectoryNotFoundException("Not able to find the CMDI lists directory.");
		...
		{
			Add(new CMDIListItem(value, definition));
		}
		...
		public CMDIListItem FindByValue(string value)
		{
			try
			{
		...
		public CMDIListItem FindByText(string text)
		{
